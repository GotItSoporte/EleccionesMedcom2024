...
router.get("/ALCALDE", async function (req, res) {
  try {
    const result = await readOracle(
      "SELECT * FROM VOTO2024.ALCALDE_GOTIT ORDER BY provincia_id asc ,distrito_id asc,votos desc "
    );
    res.json(result);
  } catch (error) {
    console.error(error);
...
router.get("/DIPUTADO", async function (req, res) {
  try {
    const result = await readOracle(
      "SELECT * FROM VOTO2024.DIPUTADO_GOTIT ORDER BY provincia_id asc,circuito_id asc ,orden_clasificacion asc, votos desc"
    );
    res.json(result);
  } catch (error) {
    console.error(error);
